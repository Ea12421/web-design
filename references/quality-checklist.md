...
## 反 AI slop 自检（完整清单见 `anti-slop.md`，逐条过）

- [ ] 无通用假人名 / startup 烂大街品牌名 / 占位头像
- [ ] 无空洞营销词（Elevate/Seamless/赋能/颠覆…），改用具体表达
- [ ] 无一眼假的完美数字；无通用步骤标签（Step 1/2/3）
- [ ] 无装饰噪音文字：版本标签 / section 编号 eyebrow / 滚动提示 / 天气地点条
- [ ] eyebrow 小标签数 ≤ ceil(section数 / 3)
- [ ] 无 3 列等大卡片；logo 墙不在 hero；hero 标题不超 viewport
- [ ] 无 `<div>` 拼的假截图 / 假产品 UI
- [ ] CTA 文字桌面端单行；无重复意图 CTA
- [ ] 无纯黑 #000；浅底无纯黑投影
- [ ] 英文文案无 em-dash（中文页面不限）
