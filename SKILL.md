...
- **反 AI slop 审计**：读 `references/anti-slop.md` 逐条过，清掉「一眼 AI 做的」痕迹（假人名/空洞动词/装饰噪音/假截图等）
- **机械检测（可选，与 anti-slop 互补）**：对生成产物跑 `npx -y impeccable detect --json --gpt <文件 / 目录 / URL>`，把报告的 anti-pattern 并入判断，并连同 JSON 结果一起交给 C4 评审。detector 抓 **CSS/HTML 机械层**（烂大街字体 / 紫色 AI 渐变 / 弹跳缓动 / 跳级标题等），anti-slop 抓 **文案 / 中文空话 / 布局**——两者不重叠。`--gpt` 顺带报 GPT/Codex 特有的味。无 node / 联网失败则跳过，不阻塞。

### C4. 独立设计评审（GAN 回环）

C3 是**自己审自己**——会手软。C4 是**让一双没参与设计的眼睛**真看一遍打分。

完整机制见 `~/.claude/独立评审机制.md`，本步只提供 rubric + 观察方式：

- **ROI 分流**（不是每次都上）：
  - 简单静态小页（个人卡片、单屏）→ C3 自审即可，跳过 C4
  - **L2+ / Landing Page / 要给客户看的 → 必走 C4**
- **观察方式**：派一个 fresh-context 子 agent，用 Preview/Chrome MCP **真把页面截图**（桌面 + 移动端各一张），看渲染结果，不是看代码。
- **rubric（直接复用本 skill 现成红线，不另造）**：
  - 设计质量 / 原创性 / 精致度 / 功能完整 —— 四维各打分
  - 硬阈值（任一不过 = 打回）：零硬编码 hex、组件状态齐全、L2+ 6 类签名动效到位、移动端无横向溢出、通过 anti-slop、未违反 Do's and Don'ts
- **回环**：不过 → 子 agent 给"具体哪屏/哪个组件不行 + 怎么改" → 改 → 重截图重评，最多 2-3 轮 → 仍不过则停下报给用户。

> 注意：C4 的子 agent 只挑刺打分，明确告诉它"你没参与设计，任务是找问题不是夸"。
...
- 通过 `references/anti-slop.md` 反 AI 味检查（无假人名 / 空洞动词 / 装饰噪音文字 / `<div>` 假截图等）
...
| `references/anti-slop.md` | 反 AI 味禁忌清单（命名/文案/装饰/布局/动效） | Phase B（写 Don'ts）+ Phase C 审计 |
